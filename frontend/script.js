...
function proceedToCheckout() {
  ...
  Object.keys(grouped).forEach((cat) => {
    const total = grouped[cat].reduce(
  ...
  // Immediately calculate delivery charge & grand total
...
function updateDeliveryCharge() {
  ...
  if (
    mainArea &&
    subArea &&
    deliveryAreas[mainArea] &&
    deliveryAreas[mainArea].subAreas[subArea]
  ) {
  ...
  // Re-render only if we are on the order summary or payment page
...
function proceedToPayment() {
  if (
    !state.orderSummary.selectedMainArea ||
...
function renderPaymentPage() {
  ...
  // Ensure grand total is up‑to‑date
  updateDeliveryCharge();

  const container = document.createElement("div");
  container.className =
    "fixed inset-0 z-50 overflow-y-auto bg-gray-50 p-4 md:p-6";
...
async function placeOrder() {
  ...
  const commissionAmount = itemsWithCommission.reduce(
    (sum, item) =>
      sum + (item.price * item.quantity * (item.commission || 0)) / 100,
    0,
  );
...
function updateNavUser() {
  ...
  if (user) {
    navUser.innerHTML = `
      <span class="text-sm font-medium text-gray-700">Hi, ${user.first_name}</span>
      <button id="navLogoutBtn" class="text-red-500 hover:text-red-700 text-sm font-medium">Logout</button>
    `;
    ...
    navUser.innerHTML = `
      <button id="navLoginBtn" class="gradient-btn text-white px-4 py-2 rounded-full text-sm font-semibold">Login</button>
    `;
...
function openLogin() {
  ...
  document.getElementById("loginTabBtn").className =
    "flex-1 py-2 text-center font-semibold text-primary border-b-2 border-primary";
  document.getElementById("signupTabBtn").className =
    "flex-1 py-2 text-center font-semibold text-gray-400 border-b-2 border-transparent";
}
...
async function verifySignupOtp() {
  ...
  if (!data) {
    showToast("Please fill details first");
    return;
  }
  try {
...
function renderRestaurantCard(restaurant) {
  const container = document.createElement("div");
  container.className =
    "restaurant-card bg-white rounded-xl shadow-md overflow-hidden cursor-pointer";
  ...
  btn.className =
    "gradient-btn text-white px-4 py-2 rounded-full text-sm w-full mt-3";
...
function renderCategoryPage() {
  ...
  const backBtn = document.createElement("button");
  backBtn.className =
    "text-2xl text-gray-600 hover:text-primary transition-colors hidden md:inline-flex items-center gap-2 mb-4";
  ...
  if (state.selectedRestaurant) {
    ...
    backToRestaurantsBtn.innerHTML =
    ...
    const menuItems = products.filter(
      (p) =>
        p.restaurant_id === state.selectedRestaurant.id &&
        p.status === "Active",
    );
  ...
  if (state.selectedCategory === "Food") {
    const foodRestaurants = restaurants.filter(
      (r) => r.category === "Food" && r.status === "Active",
    );
    ...
    const catProducts = products.filter(
      (p) => p.category === state.selectedCategory && p.status === "Active",
    );
...
function renderAccountDrawer() {
  ...
  drawer.className =
    "absolute right-0 top-0 h-full w-full max-w-sm bg-white shadow-2xl overflow-y-auto p-6";
  const header = document.createElement("div");
...
`;
    ...
    logoutBtn.className =
      "gradient-btn w-full text-white py-3 rounded-full mt-6";
    ...
    loginBtn.className =
      "gradient-btn w-full text-white py-3 rounded-full mt-4";
    ...
    care.innerHTML = `
      ...
      <p class="text-sm">✉️ support@captadelivery.com</p>
    `;
    drawer.appendChild(care);
  }
...
function renderOrderSummary() {
  ...
  // Recalculate grand total (e.g., after coming back from payment)
  updateDeliveryCharge();
  ...
  const header = document.createElement("div");
  header.className =
    "flex justify-between items-center mb-6 bg-white rounded-xl p-4 shadow-md";
  header.innerHTML = `
  ...
  Object.keys(grouped).forEach((cat) => {
    ...
    catItems.forEach((item) => {
      ...
      row.className =
        "flex justify-between items-center text-sm py-1 px-2 hover:bg-gray-50 rounded";
      row.innerHTML = `
  ...
  subtotalDiv.className =
    "flex justify-between items-center pt-2 font-semibold text-gray-700";
  ...
  mainSelect.className =
    "w-full px-4 py-2 rounded-lg border border-gray-300 focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none";
  ...
  if (
    state.orderSummary.selectedMainArea &&
    deliveryAreas[state.orderSummary.selectedMainArea]
  ) {
    const subAreas =
      deliveryAreas[state.orderSummary.selectedMainArea].subAreas;
    Object.keys(subAreas).forEach((sub) => {
      const opt = document.createElement("option");
      opt.value = sub;
      opt.textContent = `${sub} (₹${subAreas[sub]} delivery)`;
  ...
  deliveryChargeDiv.className =
    "flex justify-between items-center text-sm text-gray-600 border-t border-gray-100 pt-2 mt-2";
  ...
  totalRow.className =
    "flex justify-between items-center text-xl font-bold text-primary";
  totalRow.innerHTML = `
    <span>Grand Total</span>
    <span>₹${state.orderSummary.grandTotal}</span>
  `;
  ...
  proceedBtn.className =
    "gradient-btn w-full text-white py-3 rounded-xl font-semibold text-lg mt-4 hover:scale-[1.02] transition-transform";
  proceedBtn.textContent = "Proceed to Payment →";
  proceedBtn.addEventListener("click", proceedToPayment);
  totalSection.appendChild(proceedBtn);
  const backBtn = document.createElement("button");
  backBtn.className =
    "w-full mt-2 text-gray-600 hover:text-gray-800 font-medium py-2";
...
function renderHero() {
  const container = document.createElement("div");
  container.className =
    "bg-gradient-to-r from-blue-600 to-primary rounded-2xl mx-4 my-4 p-8 text-white";
  container.innerHTML = `
...
function renderCategoriesSection() {
  ...
  catList.forEach((cat) => {
    ...
    card.className =
      "category-card flex flex-col items-center gap-2 cursor-pointer p-3 rounded-xl bg-white hover:shadow-lg transition-all";
    ...
    if (cat.name === "Food") {
      ...
      count = products.filter(
        (p) => p.category === cat.name && p.status === "Active",
      ).length;
    }
...
function renderProductsGrid() {
  ...
  let displayProducts = filtered.filter(
    (p) => p.category !== "Food" && p.status === "Active",
  );
...
function renderCategoryModal() {
  ...
  modal.className =
    "relative bg-white rounded-2xl max-w-sm w-full max-h-[80vh] overflow-y-auto p-6";
  const header = document.createElement("div");
  ...
  categories.forEach((cat) => {
    ...
    item.className =
      "category-modal-item flex flex-col items-center gap-2 cursor-pointer p-4 rounded-xl bg-gray-50 hover:bg-primary/10 transition border border-transparent hover:border-primary";
...
function renderCartSidebar() {
  ...
  sidebar.className =
    "absolute right-0 top-0 h-full w-full max-w-md bg-white shadow-2xl overflow-y-auto";
  const header = document.createElement("div");
  ...
  if (cart.length === 0) {
    ...
    Object.keys(grouped).forEach((cat) => {
      const catHeader = document.createElement("div");
      catHeader.className =
        "font-bold text-primary text-sm uppercase tracking-wider mt-2 mb-1";
      ...
      catSubtotal.className =
        "text-right text-sm font-semibold text-gray-700 mb-3";
    ...
    const totalRow = document.createElement("div");
    totalRow.className =
      "flex justify-between font-bold text-lg pt-2 border-t border-gray-300";
...
function renderOrdersModal() {
  ...
  const modal = document.createElement("div");
  modal.className =
    "relative bg-white rounded-2xl max-w-2xl w-full max-h-[80vh] overflow-y-auto p-6";
  const header = document.createElement("div");
  header.className = "flex justify-between items-center mb-4";
  const title = document.createElement("h2");
  title.className = "text-2xl font-bold";
  ...
  if (orders.length === 0) {
    ...
    orders.forEach((order, idx) => {
      ...
      reorderBtn.className =
        "gradient-btn text-white px-4 py-1 rounded-full text-sm";
...
function renderToast() {
  ...
  toast.className =
    "fixed bottom-28 left-1/2 transform -translate-x-1/2 bg-gray-800 text-white px-6 py-2 rounded-full shadow-lg z-50 text-sm";
    ...
    searchToggleBtn.addEventListener("click", function () {
      ...
      if (mobileInput) {
        if (
          ...
          mobileInput.classList.contains("block")
        ) {
