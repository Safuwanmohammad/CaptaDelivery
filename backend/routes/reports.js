...
router.get('/commission', async (req, res) => {
  try {
    ...
    const query = `
      SELECT DATE_TRUNC('day', date) AS day, COUNT(*) AS orders,
      SUM(grand_total) AS sales, SUM(commission_amount) AS commission
      FROM orders WHERE status = 'Delivered' AND date > NOW() - INTERVAL '${interval}'
    ...
    res.json({ labels, orderCounts, salesData, commissionData, totalCommission, totalSales, totalOrders });
  } catch (err) {
...
router.get('/restaurant/:id', async (req, res) => {
  try {
    ...
    const query = `
      SELECT o.order_id, o.date, o.grand_total, o.commission_amount, o.delivery_charge, o.rain_fare, o.items
      FROM orders o WHERE o.status = 'Delivered'
      AND EXISTS (SELECT 1 FROM jsonb_array_elements(o.items) AS item WHERE (item->>'restaurant_id')::int = $1)
    ...
    const ordersDetail = result.rows.map(row => {
      ...
      return { orderId: row.order_id, date: row.date, grandTotal, commission, delivery, rain, restaurantRevenue: grandTotal - commission - delivery - rain };
    });
    res.json({ totalOrders: result.rows.length, totalRevenue, totalCommission, restaurantRevenue: totalRevenue - totalCommission - ordersDetail.reduce((s, o) => s + (o.delivery || 0) + (o.rain || 0), 0), orders: ordersDetail });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});
...
router.put('/restaurant/:id/toggle', async (req, res) => {
  try {
    ...
    const result = await pool.query('UPDATE restaurants SET status = $1 WHERE id = $2 RETURNING *', [status, id]);
...
router.get('/category-commission', async (req, res) => {
  try {
    ...
    const query = `SELECT items, commission_amount, grand_total FROM orders WHERE status = 'Delivered' AND date > NOW() - INTERVAL '${interval}'`;
    const result = await pool.query(query);
